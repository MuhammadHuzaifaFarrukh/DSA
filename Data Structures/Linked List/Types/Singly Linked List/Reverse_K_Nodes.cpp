...
public:
    ...
    // This problem asks only to reverse the K-Nodes in K Groups , if nodes left are smaller than K , then don't reverse them.
    ...
    // We have total groups upto N/k so we can say our time is : k*N/k = N.
    ...
    Node *reverseKGroup(Node *head, int k)
    {
        ...
        while (first->next) // This checks our end of list.
        {
            ...
            while (check && count < k)      // We also need to check if we have enough elements to reverse.
            {
                check = check->next;
                count++;
            }
            ...
            while (x )      // No need to use x && curr as we don't need to reverse nodes fewer than 'k'
            {
                front = curr->next;
                curr->next = prev;
                prev = curr;
