...
// So for handling it we can either do k>=n-1 and then find the max element ourselves or we can just write k = min(k,n-1) and then skip this whole if-part.
...
int findWinningPlayer(vector<int> &skills, int k)
{
    ...
    // We can skip the upper part (though its good to have distinguished)
    // By writing below statement , k will be capped in 0 to n-1 and our deque loop will find it.
    // k = min(k,n-1);
