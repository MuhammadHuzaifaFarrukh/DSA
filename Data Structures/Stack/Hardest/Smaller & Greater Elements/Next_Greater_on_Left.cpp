...
// This problem asks to find the greater number of each element nums[i] to its Left side.
...
// 2) We keep those elements' indexes in the stack whose next greater we have to find (in descending order).
...
vector<int> left_next_greater(vector<int> &nums)
{
    stack<int> st;
...
vector<int> left_next_greater2(vector<int> &nums)
{
    stack<int> st;
    vector<int> ans(nums.size(), -1);
...
int main()
{
    vector<int> v = {4, 13, 11, 4, 5, 9, 7, 6, 6};
    v = left_next_greater2(v);
    for (int n : v)
    {
        cout << n << " ";
    }
