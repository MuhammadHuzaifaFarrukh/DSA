...
int main()
{
    ...
    pop_heap(v.begin(), v.end()); // It swaps the largest element in vector with last element, the last element is still accessible. (Just like Sift Down).
    ...
    push_heap(v.begin(), v.end()); // But this then checks that last element to be swapped with the front one or not. (Just like Sift Up)
