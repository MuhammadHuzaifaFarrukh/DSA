...
public:
    ...
    // Total Complexity is O(n log n) while in making heap
    // Uses O(logn) in insertion
    // Its Time Complexity Itself is  : O(logn)
    // We can find Time Complexity of each operation by visualizing them in the form of tree better than as linear.

    // For .push() : It inserts the element at the last available position of the array and then checks for that element whether it is at its correct position or not.
    // If its at its correct position then stops otherwise it compares from its parents and keeps going up.
    void StepUp(int index) // Also known as Sift/Shift Up
    {
        /*
    ...
    // Heapify also called Step Down / Sift / Shift Down.

    // For .pop() : .pop() replaces the last element with top most element and the last element is deleted.
    // Then it performs heapify for the topmost element to bring it to its correct position by comparing to its children.
    ...
    // We take each index 'i' and move it to its correct upward position by comparison of each child with its parent node using StepUp().
    void make_heap_step_up() // Makes an array or a vector into heap order
    {
        ...
        // We start from the first child and "insert" every element into the existing heap structure to its left.
        // For the first element , it doesn't have any parent , so that's why we check from the 2nd index 1.
    ...
    // We take each node 'i' and take it to its correct position downwards by comparison of each parent with its child.
    ...
    // The 1st and 2nd highest elements can be same in both make_heaps() but other maxima will be different usually.
    ...
    // This swaps Root node (0-Index) and last node.
    // Then uses Step-Down Approach to bring the element down if it is smaller than its child nodes.
    // Uses O(logn)
    ...
    // Then we swap the root node with the last valid index , and apply heapify for the root node element so its put to its correct place.
    // We also use a temporary size variable because we need original size variable to be used as decremented by 1 in our Heapify so we can place a border between our sorted and unsorted range.
    // The elements that are placed behind (swapped) are sorted so that's why we don't include them in our original size range and hence we decrement our original size variable.
