...
int evaluateBoard(const vector<vector<char>> &board, int N);
bool isBoardFull(const vector<vector<char>> &board, int N);
int minimaxAlphaBeta(vector<vector<char>> &board, int N, bool isAI, int alpha, int beta);
void makeBestAIMove(vector<vector<char>> &board, int N);
...
int main()
{
    ...
    if (N < 3)
    {
        cout << "Board size must be at least 3x3.\n";
        return 0;
    }

    if (N > 3)
    {
        cout << "\n[WARNING]: Running full Minimax without depth limits on N = " << N
             << " will freeze or take a very long time!\n\n";
    }

    vector<vector<char>> board(N, vector<char>(N, ' '));

    cout << "=== Tic-Tac-Toe (" << N << "x" << N << "): You (X) vs Alpha-Beta AI (O) ===\n";
    ...
    while (true)
    {
        ...
        makeBestAIMove(board, N); // Finds best move using Alpha-Beta and places 'O'
...
void printBoard(const vector<vector<char>> &board, int N)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        if (i < N - 1)
        {
            for (int j = 0; j < N; j++)
            {
                cout << "\xC4\xC4\xC4"; // horizontal bar (─)
                if (j < N - 1)
                {
                    cout << "\xC5"; // cross intersection (┼)
                }
            }
            cout << "\n";
        }
    ...
    for (int r = 0; r < N; r++)
    {
        for (int c = 0; c < N; c++)
        {
            if (board[r][c] == ' ')
            {
                ...
                int moveVal = minimaxAlphaBeta(board, N, false, alpha, beta);
...
int minimaxAlphaBeta(vector<vector<char>> &board, int N, bool isAI, int alpha, int beta)
{
    int score = evaluateBoard(board, N);
    ...
    if (isAI)
    {
        ...
        for (int r = 0; r < N; r++)
        {
            for (int c = 0; c < N; c++)
            {
                ...
                {
                    board[r][c] = 'O';
                    int currentScore = minimaxAlphaBeta(board, N, false, alpha, beta);
                    board[r][c] = ' '; // Backtrack
    ...
    else
    {
        ...
        for (int r = 0; r < N; r++)
        {
            for (int c = 0; c < N; c++)
            {
                ...
                {
                    board[r][c] = 'X';
                    int currentScore = minimaxAlphaBeta(board, N, true, alpha, beta);
                    board[r][c] = ' '; // Backtrack
