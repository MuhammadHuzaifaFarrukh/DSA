...
Note: Return the total number of paths.
If no valid path exists, return an empty list.
*/

// Forward Declaration :
int total(vector<vector<int>> &maze, vector<vector<bool>> &visited, int i, int j, int n);
...
// If our i,j == n-1 then it means we have reached successfully to one answer and we return its count to 1.
...
// When we reach a deadend , only then we go back and undo our visited track record.
...
int ratInMaze(vector<vector<int>> &maze)
{
    ...
    return total(maze, visited, 0, 0, maze.size());
}
...
int total(vector<vector<int>> &maze, vector<vector<bool>> &visited, int i, int j, int n)
{
    ...
    if (valid(i - 1, j, n) && maze[i - 1][j] && !visited[i - 1][j])
    {
        ...
        count += total(maze, visited, i - 1, j, n);
    }
    ...
    if (valid(i, j - 1, n) && maze[i][j - 1] && !visited[i][j - 1])
    {
        ...
        count += total(maze, visited, i, j - 1, n);
    }
...
int col[4] = {0, 0, -1, 1};
string dir = "UDLR";
int total2(vector<vector<int>> &maze, vector<vector<bool>> &visited, int i, int j, int n)
{
    if (i == n - 1 && j == n - 1)
    {
        return 1;
    ...
    for (int k = 0; k < 4; k++)
    {
        if (valid(i + row[k], j + col[k], n) && maze[i + row[k]][j + col[k]] && !visited[i + row[k]][j + col[k]])
        {
            ...
            count += total2(maze, visited, i + row[k], j + col[k], n);
        }
    }
    visited[i][j] = 0;
    return count;
